...
def empty_token_matrix() -> np.ndarray:
    return np.zeros((0, PLANNER_TOKEN_DIM), dtype=np.float32)
...
def ensure_bundle(raw_bundle: Any) -> Dict[str, np.ndarray]:
    ...
    return {
        "world_tokens": np.asarray(raw_bundle.get("world_tokens", empty_token_matrix()), dtype=np.float32),
        ...
        "hand_tokens": np.asarray(raw_bundle.get("hand_tokens", empty_token_matrix()), dtype=np.float32),
        "hand_mask": np.asarray(raw_bundle.get("hand_mask", empty_bool_vector()), dtype=np.bool_),
        "action_tokens": np.asarray(raw_bundle.get("action_tokens", empty_token_matrix()), dtype=np.float32),
        ...
        "global_scalars": np.asarray(
            raw_bundle.get("global_scalars", np.zeros((PLANNER_GLOBAL_DIM,), dtype=np.float32)),
            dtype=np.float32,
        ).reshape(-1),
    }


def bundle_to_torch(raw_bundle: Any, device: torch.device) -> Dict[str, torch.Tensor]:
    bundle = ensure_bundle(raw_bundle)
    return {
...
def pad_bundle_batch(raw_bundles: Sequence[Any], device: torch.device) -> Dict[str, torch.Tensor]:
    bundles = [ensure_bundle(item) for item in raw_bundles]
    if not bundles:
    ...
    world_tokens = torch.zeros((batch, max_world, PLANNER_TOKEN_DIM), dtype=torch.float32, device=device)
    ...
    hand_tokens = torch.zeros((batch, max_hand, PLANNER_TOKEN_DIM), dtype=torch.float32, device=device)
    hand_mask = torch.zeros((batch, max_hand), dtype=torch.bool, device=device)
    action_tokens = torch.zeros((batch, max_action, PLANNER_TOKEN_DIM), dtype=torch.float32, device=device)
    ...
    global_scalars = torch.zeros((batch, PLANNER_GLOBAL_DIM), dtype=torch.float32, device=device)
    ...
    for row, item in enumerate(bundles):
        ...
        take = min(int(g.size), PLANNER_GLOBAL_DIM)
        if take > 0:
...
def token_from_features(
    ...
    feature_dim: int = PLANNER_TOKEN_DIM,
) -> np.ndarray:
    vec = np.zeros((feature_dim,), dtype=np.float32)
    vec[0] = float(type_id) / 16.0
...
def planner_aux_layout(
    num_milestones: int,
    num_awards: int,
    opportunity_limit: int = PLANNER_OPPORTUNITY_LIMIT,
) -> Dict[str, slice]:
